...
public class WeatherActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        swipeRefresh.setColorSchemeColors(getResources().getColor(R.color.white));//设置进度条的颜色
        ...
        {
            Weather weather = Utility.handleWeatherResponse(weatherString);//解析JSON
            /**
             * 展示天气
             */
    ...
    public void requestWeather(final String weatherId)
    {
        ...
        HttpUtil.sendOkHttpRequest(weatherUrl, new Callback()
        {
            ...
            public void onFailure(Call call, IOException e)
            {
                runOnUiThread(new Runnable()
                {
                    ...
                    public void run()
                    {
                        ...
                        swipeRefresh.setRefreshing(false);//表示刷新动作结束，隐藏刷新进度条
                    }
                });
            }
            ...
            {
                final String responseText = response.body().string();//获取字符串格式
                Log.i(TAG, responseText);
                final Weather weather = Utility.handleWeatherResponse(responseText);//处理，生成Weather对象
                runOnUiThread(new Runnable()
                {
                    @Override
                    public void run()
